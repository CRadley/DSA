...
class MaxHeap:
    ...
    def _heapify_down(self, index: int):
        ...
        if self.length > right_index:
            max_child = max(self._heap[left_index], self._heap[right_index])
            max_child_index = (
                left_index if max_child == self._heap[left_index] else right_index
            )
        else:
            max_child = self._heap[left_index]
            max_child_index = left_index
        if max_child > self._heap[index]:
